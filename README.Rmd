...
`scheduler` is a grammar of recurring calendar events, implemented in an R 
package. It enables users to easily create and work with schedules, even in 
cases when the pattern of events is irregular. `scheduler` does this by 
...
`scheduler` implement it for them.
...
made in the [issues tab](https://github.com/jameslairdsmith/scheduler/issues) 
of the Github repo.
...
their paper[^1] on `lubridate` inspired me to create `scheduler` and pointed 
