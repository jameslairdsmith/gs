...
`scheduler` is an R package for working with recurring calendar events. When a
set of these events recur in a pattern, we call it a 'schedule'.

Schedules can sometimes be frustrating to create, especially when the pattern 
of events is irregular. `scheduler` overcomes this by providing a set of 
simple to understand functions for making schedules along with functions for 
combining them into more complex ones. 
...
[contributors][contributors] to the `lubridate` package. `scheduler` relies 
...
their paper[^1] on `lubridate` pointed me in Fowler's direction:
