...
occuring on the second day of every year. But in our case we want to create 
...
created a schedule of all the days occuring on the first day of the year.
...
In this example you can validate this simply inspecting the dates.
...
the prefix 'on_' or 'in_'. Among other things, this will make the syntax
...
For example, the `on_mday()` function creates a schedule of events occuring on
...
a schedule of events occuring on both the inputs. For example 
`on_wday("Mon", "Wed", "Friday")` creates a schedule of events occuring on all
of those weekdays. For convenience the `on_weekday()` and `on_weekday()` 
...
instead of focussing on holidays, you instead wanted to get a schedule of 
...
# Nth occurances within periods

One of the most common schedule patterns are when events take place on the nth
occurance of another schedule within a given period. For example, in the 
United States, Martin Luther King Jr. Day is celebrated on the third Monday in
January.

This date rule has a few components. 'Monday', 'January' and 'third occurance
...
To find the third occurance of a scheduled event within a period `gs` provides 
...
first argument. Positive integers encode the nth occuring event within the 
period; negative integers encode the nth last occuring events of the period.

As a somewhat bizzare example, say that you play for a football team whose
