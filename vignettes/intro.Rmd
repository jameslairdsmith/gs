...
have specified. Let's expand these bounderies so we get more occurrances of
New Year's Day. Here I extend the bounderies from the start of 1995 to the 
end of 2005.
...
Because this is a little cumbersom to write, `schedule_days()` allows you to
abbreviate `to` and `from` to only the numeric years you wish to use as 
bounderies. These go from the start of the `from` year to the end of the `to`
